...
void run(unsigned thread_id, Address public_ip, Address private_ip,
  ...
  while (true) {
    ...
    if (join_addr_keyset_map.size() != 0) {
      ...
      logger->info("Join key set map size is {}.", join_addr_keyset_map.size());

      // assemble gossip
      AddressKeysetMap addr_keyset_map;
      for (auto& join_pair : join_addr_keyset_map) {
        ...
        logger->info("Size of key set is {}.", key_set.size());
        logger->info("About to erase {} key from a map of size {}.", sent_keys.size(), join_pair.second.size());
        for (const Key& key : sent_keys) {
          key_set.erase(key);
        }
        logger->info("Map size is now {}.", join_pair.second.size());
      ...
      if (join_addr_keyset_map.size() == 0) {
        for (const std::string& key : join_remove_set) {
          logger->info("Removing key {} locally.", key);
