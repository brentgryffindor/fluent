...
void key_shipping_response_handler(const string& serialized, map<string, pair<set<Address>, Address>>& pending_key_shipping_map,
  ...
  for (const auto& function_causal_tuple_pair : response.function_causal_tuple_pairs()) {
    // merge into conservative store
    auto cid_function_pair = std::make_pair(response.client_id(), function_causal_tuple_pair.function_name());
    const Key& key = function_causal_tuple_pair.tuple().key();
    auto lattice = std::make_shared<CrossCausalLattice<SetLattice<string>>>(
            to_cross_causal_payload(deserialize_cross_causal(function_causal_tuple_pair.tuple().payload())));
    if (conservative_store[cid_function_pair].find(key) == conservative_store[cid_function_pair].end()) {
      ...
      conservative_store[cid_function_pair][key] = causal_merge(conservative_store[cid_function_pair][key], lattice);
    }
  }
  pending_key_shipping_map[response.client_id()].first.erase(response.cache_address());
  ...
  if (pending_key_shipping_map[response.client_id()].first.size() == 0) {
    ...
    scheduler_response.set_cache_address(cct.causal_cache_scheduler_key_shipping_request_connect_address());
    // send response
    string resp_string;
    scheduler_response.SerializeToString(&resp_string);
    kZmqUtil->send_string(resp_string, &pushers[pending_key_shipping_map[response.client_id()].second]);
    // GC
