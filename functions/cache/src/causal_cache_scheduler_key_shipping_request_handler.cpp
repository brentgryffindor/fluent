...
void scheduler_key_shipping_request_handler(const string& serialized, map<string, pair<set<Address>, Address>>& pending_key_shipping_map,
  ...
  pending_key_shipping_map[request.client_id()].second = request.response_address();
  ...
  for (const auto& per_cache_function_key_pair : request.per_cache_function_key_pairs()) {
    pending_key_shipping_map[request.client_id()].first.insert(per_cache_function_key_pair.cache_address());
    ...
    kZmqUtil->send_string(req_string, &pushers[per_cache_function_key_pair.cache_address()]);
  }
}