#ifndef COMMON_HASH_UTIL_HPP_
#define COMMON_HASH_UTIL_HPP_
...
struct HashCombine {
  template <typename T>
  std::size_t operator()(std::size_t acc, const T& x) const {
...
struct Hash<std::vector<T>> {
  std::size_t operator()(const std::vector<T>& xs) const {
